...
function automatic logic [`word_size-1:0] decode_imm(instr32 instr, instr_format format);
    case(format)
        ...
        b_format : return { {(`word_size - 13){instr[31]}}, instr[31], instr[7], instr[30:25], instr[11:8], 1'b0 };
        j_format : return { {(`word_size - 21){instr[31]}}, instr[31], instr[19:12], instr[20], instr[30:21], 1'b0 };
...
//if 1 on branches means branch taken
function automatic ext_operand execute(
    ...
    case (op_q)
        ...
        q_branch: begin
            case (f3)
                f3_beq: result = ext_operand'(rd1 == rd2);              
                f3_bne: result = ext_operand'(rd1 != rd2);                
                f3_blt: result = ext_operand'($signed(rd1) < $signed(rd2)); 
                f3_bge: result = ext_operand'($signed(rd1) >= $signed(rd2)); 
                f3_bltu: result = ext_operand'(rd1 < rd2);                  
                f3_bgeu: result = ext_operand'(rd1 >= rd2);           
                default: result = ext_operand'(0);
            endcase
        end
        q_jal: begin
            result = {1'b0, pc } + 4;
...
function automatic word get_next_pc(
    word pc, 
    word imm, 
    word rd1, 
    opcode_q op_q, 
    ...
    case (op_q)
        q_jal:    return pc + imm;
        q_jalr:   return (rd1 + imm) & ~32'h1;
        q_branch: return take_branch ? (pc + imm) : (pc + 4);
        default:  return pc + 4;
    endcase
endfunction
